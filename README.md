...
def detect_handwriting(image_path):
    ...
    for contour in text_contours:
        ...
        cv2.rectangle(img_copy, (x, y), (x + w, y + h), (190, 100, 130), 2)
...
```
![output2](image-1.png)
## III)Perform object detection with label 
```py
...
for ClassInd,conf,boxes in zip(ClassIndex.flatten(),confidence.flatten(),bbox):
    ...
    cv2.putText(img,classLabels[ClassInd-1],(boxes[0]+10,boxes[1]+50),font,fontScale=font_scale,color=(255,0,0),thickness=1)
plt.imshow(cv2.cvtColor(img,cv2.COLOR_BGR2RGB))
```
![output3](image-2.png)
## RESULT:
